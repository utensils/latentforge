<!-- AUTO-GENERATED by docs/scripts/gen-api-docs.py — do not edit by hand -->
<!-- Source: src/latentforge/tools.py -->

# MCP Tools Reference

LatentForge registers the following tools with the Claude agent via a local MCP server (`create_sdk_mcp_server`). Each tool is an async Python function decorated with `@tool(name, description, params_schema)` in [`src/latentforge/tools.py`](https://github.com/utensils/latentforge/blob/main/src/latentforge/tools.py).

There are **22 tools** across 10 functional groups. The agent also has the built-in `Read` (with vision), `Write`, and `Bash` tools.
